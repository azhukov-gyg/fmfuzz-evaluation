#!/usr/bin/env python3
"""
Prepare Commit Fuzzer (Sancov-enabled version) for Z3
...
class PrepareCommitAnalyzer:
    ...
    def is_z3_function(self, signature: str) -> bool:
        ...
        try:
            ...
            if head.startswith('std::') or head.startswith('__') or head.startswith('__gnu_cxx::'):
                return False
            # Include any functions within z3-related namespaces
            # Z3 uses various namespaces like z3::, nlsat::, smt::, arith::, etc.
            z3_namespaces = ['z3::', 'nlsat::', 'smt::', 'arith::', 'bv::', 'sat::', 'polynomial::',
                           'datalog::', 'qe::', 'spacer::', 'opt::', 'tactic::']
            for ns in z3_namespaces:
                if ns in head:
                    return True
    ...
    def get_commit_functions(self, commit_hash: str) -> tuple[List[str], List[str], Dict[str, FunctionInfo]]:
        ...
        for file_path, changed_lines in changed_files_lines.items():
            ...
            if after_funcs:
                cvc5_funcs = [f for f in after_funcs if self.is_z3_function(f.signature)]
