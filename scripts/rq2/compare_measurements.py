#!/usr/bin/env python3
"""
Compare measurement results across baseline and variant1.

Usage:
    python compare_measurements.py <measurements_dir> --output <output_file>

    measurements_dir: Directory containing measurements/{baseline,variant1}/*.json
...
def compare_measurements(measurements_dir: str) -> dict:
    """Compare measurements across baseline and variant1."""
    ...
    # Find common commits
    common_commits = set(baseline.keys()) & set(variant1.keys())
    print(f"Common commits: {len(common_commits)}", file=sys.stderr)
    ...
    for commit in sorted(common_commits):
        ...
        # Count functions covered (non-zero calls)
        ...
        v1_only = sum(1 for f in all_functions if
                      ...
                      b.get('function_counts', {}).get(f, 0) == 0)
    ...
    summary['winner_by_calls'] = max(variants, key=lambda v: summary[f'avg_{v}_calls'])
    summary['winner_by_coverage'] = max(variants, key=lambda v: summary[f'avg_{v}_coverage_pct'])
    summary['winner_by_exclusive'] = max(variants, key=lambda v: summary[f'total_{v}_exclusive'])
    summary['winner_by_line_coverage'] = max(variants, key=lambda v: summary[f'avg_{v}_line_coverage_pct'])
    summary['winner_by_branch_coverage'] = max(variants, key=lambda v: summary[f'avg_{v}_branch_coverage_pct'])
...
def print_summary(comparison: dict):
    ...
    if 'summary' in comparison:
        ...
        print(f"\n📊 METRIC 1: Total Function Calls", file=sys.stderr)
        ...
        if 'v1_vs_baseline_ratio' in s:
            print(f"  Ratio: Variant1 is {s['v1_vs_baseline_ratio']:.2f}x baseline", file=sys.stderr)

        print(f"\n📊 METRIC 2: Coverage Breadth %", file=sys.stderr)
        ...
        print(f"\n📊 METRIC 3: Exclusive Functions", file=sys.stderr)
        ...
        print(f"\n📊 METRIC 4: Calls per Recipe", file=sys.stderr)
        ...
        print(f"\n📊 METRIC 5: Line Coverage %", file=sys.stderr)
        ...
        print(f"\n📊 METRIC 6: Branch Coverage %", file=sys.stderr)
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare measurement results across baseline and variant1"
    )
    parser.add_argument(
        "measurements_dir",
        help="Directory containing measurements/{baseline,variant1}/*.json"
    )
    parser.add_argument(
        "--output", "-o",
        required=True,
